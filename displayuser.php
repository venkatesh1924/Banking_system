    ...
    if (($amount)<0)
   {
    ...
    // constraint to check zero values
         ...
         echo "</script>";
                ...
                $sql = "UPDATE customers set currrentbalance=$newbalance where id=$to";
                mysqli_query($conn,$sql);
                
                $sender = $sql1['name'];
                $receiver = $sql2['name'];
                $sql = "INSERT INTO transactions(`SenderName`, `ReceiverName`, `Amount`) VALUES ('$sender','$receiver','$amount')";
                $query=mysqli_query($conn,$sql);
